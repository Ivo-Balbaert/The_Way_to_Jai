...
1. The compiler is written in C++. There are no header files in Jai, and no forward declarations. Also, builds can be controlled from within the language, the process is the same for all platforms (see § 30); make-files or any other external build-systems are not needed. Jai has extensive compile time meta programming functionality, even to the point of being able to run programs at compile time (see § 3.2.4). It also has compile time polymorphism (see § 22-23), and a powerful macro system ( see § 26.5).

The goal is to compile 1 million lines of code in \< 1s (from scratch, without delta builds). The official compilation speed of the compiler is 250,000 lines of code per second using the x64 backend, according to the CHANGELOG of beta 0.0.045 (using the Sokoban style game _Order of the Sinking Star_ as a benchmark).

2. Explicit control over optimization and performance characteristics: things like inlining code (see § 17.8) and LLVM optimizations (see § 30.4.8). Jai also effectively uses pointers and it has an implicit context system to switch memory allocation schemes. For simple serial code (math on scalar values) Jai is almost equivalent in speed to C. For SIMD processing, the generated code is 4-8% slower than C (see [Benchmarks](https://github.com/Jai-Community/Jai-Community-Library/wiki/Snippets-and--Benchmarks#benchmarks)).
...
C++ is from the 1980's, and it has accumulated so much concepts which complicate things and sometimes work against each other. Language concepts should be _orthogonal_, not conflicting with each other.

Jai also has macros, but these are not preprocessor macros like C/C++; they are more like Lisp macros.
...
Jai obviously belongs to the C-family of languages, together with C++, Objective C, Java, C#, and so on. For example: it uses semicolons ; to mark the end of a code-line, and encloses a block of code lines within curly braces { }. However, it clearly favours a simpler and more enjoyable syntax than C/C++.
...
Jai is build with concepts that guarantee high performance.
...
A _closed beta program_ (the compiler/docs were sent to a limited number of people) started Dec 21 2019: this was the first beta with 10 people. Apr 2020: 20 people.    
...
On Dec 2025, yhe beya reached v 0.2.024, reaching around 800 developers.
...
The business-model is based on the commercial release of games written in Jai by the Thekla company, starting with the game _Order of the Sinking Star_.  

Jai will be released gradually, in this order:  
- A closed beta release (from Dec 2019 until ??)
- An open binary distribution release and a release of the game engine written in Jai.  This will occur not long after the commercial release of the Sokoban game.
...
A Discord channel exists called [SB (Secret Beta)](https://discord.gg/wB52e2ND)
...
2025 Dec 12:    800 people enrolled in the closed beta